...
(*
  ...
  Description: Unified generator for didactic materials for solving linear systems
               via elimination and substitution methods.
*)
...
GenElimination::usage =
    "GenElimination[diff, mode, opts] vygeneruje príklad riešenia sústavy lineárnych rovníc eliminačnou metódou (sčítaním rovníc).\n\n" <>
...
GenSubstitution::usage =
    "GenSubstitution[diff, mode, opts] vygeneruje príklad riešenia sústavy lineárnych rovníc dosadzovacou (substitučnou) metódou.\n\n" <>
        "diff: \"EASY\" (2x2), \"MEDIUM\" (3x3), \"HARD\" (3x3)\n" <>
        "mode: \"TASK\", \"TASK_RESULT\", \"TASK_STEPS_RESULT\"\n" <>
        "opts: Visualization -> True|False, SolutionType -> Automatic|\"ONE\"|\"NONE\"|\"INFINITE\"";
...
GenSubstitution::baddiff = "Neplatná obtiažnosť `1`. Použi \"EASY\"|\"MEDIUM\"|\"HARD\".";
GenSubstitution::badmode = "Neplatný režim `1`. Použi \"TASK\"|\"TASK_RESULT\"|\"TASK_STEPS_RESULT\".";
GenSubstitution::badst   = "Neplatný typ riešenia `1`. Použi Automatic|\"ONE\"|\"NONE\"|\"INFINITE\".";
GenSubstitution::fail    = "Nepodarilo sa vygenerovať vhodný príklad.";

Begin["`Private`"];

Internal`$ContextMarks = False;

(* ~-~-~ VALIDATION & COMMON HELPERS ~-~-~ *)
...
printCellStyle[expr_, style_String] :=
    If[inNotebookQ[], CellPrint @ Cell[expr, style, ShowStringCharacters -> False], Print[expr]];
...
DimensionByDifficulty[_method_, diff_] := Switch[diff, "EASY", 2, "MEDIUM", 3, "HARD", 3, _, 3];
...
(* ~-~-~ ELIMINATION STEPS (Renamed with 'elim' prefix) ~-~-~ *)

elimStepsOne2[A_, b_, vars_] := Module[
  {data, content, m1, m2, elimVarStr, sumRHS, sumRow, keepIdx, elimIdx, keepVar, elimVar, stepsY, valKeep, valElim,
    stepsSub, rowOrig, rhsOrig, valProd, rhsRem},
  ...
  AppendTo[stepsSub, {formatSubstLHS[rowOrig, vars, <|keepVar -> valKeep|>, elimVar, True], rhsOrig, addNote[-valProd]}];

  rhsRem = rhsOrig - valProd;
  If[rowOrig[[elimIdx]] =!= 1, AppendTo[stepsSub, {rowOrig[[elimIdx]] elimVar, rhsRem, divNote[rowOrig[[elimIdx]]]}]];
  ...
  content = addCorrectnessCheck[content, A, b, vars, If[elimIdx == 1, {valElim, valKeep}, {valKeep, valElim}]];
  <|"Content" -> content, "Solution" -> If[elimIdx == 1, {valElim, valKeep}, {valKeep, valElim}]|>
];

elimStepsSingular2[A_, b_, vars_, kind_String, includeConclusion_ : True] := Module[
  ...
  introText = Switch[kind,
    "NONE", "Sčítaním rovníc overíme konzistenciu sústavy. Ak vznikne nepravdivá rovnosť 0 ≠ k, sústava nemá riešenie.",
    "INFINITE", "Rovnice sčítame. Ak vyjde pravdivá rovnosť 0 = 0, znamená to, že jedna rovnica je násobkom druhej a sústava má nekonečne veľa riešení.",
    _, "Sčítame rovnice."
  ];
  AppendTo[content, introText];
  AppendTo[content, renderAddition2[rowMod, rhsMod, vars]];

  rhsLine = Switch[kind, "NONE", sumRHS, "INFINITE", 0, _, sumRHS];
  AppendTo[content, alignedEquations[{{0, rhsLine, ""}}]];

  If[includeConclusion,
    AppendTo[content, makeStepHeader["Záver"]];
    conclText = Switch[kind,
      "NONE", "Sústava nemá riešenie, pretože sme dostali nepravdivú rovnosť 0 ≠ " <> ToString[sumRHS] <> ".",
      "INFINITE", "Sústava má nekonečne veľa riešení, pretože sme dostali pravdivú rovnosť 0 = 0.",
      _, ""
    ];
...
elimStepsNone2[A_, b_, vars_, includeConclusion_ : True] := elimStepsSingular2[A, b, vars, "NONE", includeConclusion];
elimStepsInfinite2[A_, b_, vars_, includeConclusion_ : True] := elimStepsSingular2[A, b, vars, "INFINITE", includeConclusion];

elimStepsOne3[A_, b_, vars_, data_ : <||>] := Module[
  ...
  If[red["twoCombosQ"], AppendTo[content, "Dostali sme dve nové rovnice s dvoma neznámymi."],
    AppendTo[content, "Z jednej dvojice rovníc sme elimináciou získali jednu novú rovnicu a druhá rovnica bola už v zadaní bez vyrušovanej premennej. Spolu tvoria sústavu 2×2."]
  ];

  AppendTo[content, alignedEquations[{{formatEquationLHS[A2[[1]], "", remVars], b2[[1]], ""}, {formatEquationLHS[A2[[2]], "", remVars], b2[[2]], ""}}]];

  sol2x2 = elimStepsOne2[A2, b2, remVars];
  If[sol2x2 === $Failed, Return[$Failed]];

  Module[{c2 = sol2x2["Content"], pos},
    pos = FirstPosition[c2, Style[s_String, Bold, ___] /; StringContainsQ[s, "Skúška správnosti"], Missing["NotFound"]];
    If[pos === Missing["NotFound"], content = Join[content, c2], content = Join[content, Take[c2, pos[[1]] - 1]]];
  ];
  stepsCounter--;
  ...
  If[coeffU === 1, AppendTo[subSteps, {tf[finalVar], tf[rhsShift], ""}],
  ...
  content = addCorrectnessCheck[content, A, b, vars, Table[If[i == red["elimCol"], finalVal, solMap[vars[[i]]]], {i, 1, 3}]];
  <|"Content" -> content, "Solution" -> Table[If[i == red["elimCol"], finalVal, solMap[vars[[i]]]], {i, 1, 3}]|>
];

elimStepsSingular3[A_, b_, vars_, kind_String, data_ : <||>] := Module[
  ...
  headerText = Switch[kind,
    ...
    _, "Riešime odvodenej sústavu 2×2."
  ];
  AppendTo[content, headerText];

  AppendTo[content, alignedEquations[{{formatEquationLHS[A2[[1]], "", remVars], b2[[1]], ""}, {formatEquationLHS[A2[[2]], "", remVars], b2[[2]], ""}}]];

  sol2x2 = elimStepsSingular2[A2, b2, remVars, kind, False];

  If[sol2x2 === $Failed,
    midText = Switch[kind, "NONE", "Po úpravách dostávame spornú rovnicu (napr. 0 = k). Preto sústava nemá riešenie.", "INFINITE", "Sústava má nekonečne veľa riešení.", _, ""];
...
elimStepsNone3[A_, b_, vars_, data_ : <||>] := elimStepsSingular3[A, b, vars, "NONE", data];
elimStepsInfinite3[A_, b_, vars_, data_ : <||>] := elimStepsSingular3[A, b, vars, "INFINITE", data];
...
(* ~-~-~ SUBSTITUTION STEPS (Renamed with 'subst' prefix) ~-~-~ *)

substStepsOne2[A_, b_, vars_] := Module[
  {content = {}, rI, cI, solveData, substRule, elimVar, remIdx, remVar, newRow, newRHS, valRem, valElim, finalSol, filledExpr,
    res, resRows, coeff},
  ...
  If[PossibleZeroQ[newRow[[1]]], Return[$Failed]];
  ...
  filledExpr = solveData["Expr"] /. remVar -> valRem;
...
substStepsSingular2[A_, b_, vars_, kind_String, includeConclusion_ : True] := Module[
  {content = {}, rI, cI, solveData, substRule, elimVar, remVar, res, newRow, newRHS, introText, conclText},
  ...
  If[res["Content"][[-1, 1]] =!= tf[0], AppendTo[content, alignedEquations[{{0, tf[newRHS], ""}}]]];

  If[includeConclusion,
    AppendTo[content, makeStepHeader["Záver"]];
    conclText = Switch[kind,
      "NONE", "Sústava nemá riešenie, pretože sme dostali nepravdivú rovnosť 0 ≠ " <> ToString[tf[newRHS]] <> ".",
      "INFINITE", "Sústava má nekonečne veľa riešení, pretože sme dostali pravdivú rovnosť 0 = 0.",
      _, ""
    ];
    AppendTo[content, conclText];
...
substStepsNone2[A_, b_, vars_, includeConclusion_ : True] := substStepsSingular2[A, b, vars, "NONE", includeConclusion];
substStepsInfinite2[A_, b_, vars_, includeConclusion_ : True] := substStepsSingular2[A, b, vars, "INFINITE", includeConclusion];

substStepsOne3[A_, b_, vars_, data_ : <||>] := Module[
  {red, content = {}, A2, b2, remVars, sol2x2, solMap, finalVal, substExpr, elimVar, valElim, filledExpr},
  ...
  AppendTo[content, makeStepHeader["Riešenie odvodenej sústavy 2×2"]];
  AppendTo[content, "Dostali sme sústavu dvoch rovníc s dvoma neznámymi. Vyriešime ju (tu pokračujeme substitúciou)."];

  AppendTo[content, alignedEquations[{{formatEquationLHS[A2[[1]], "", remVars], b2[[1]], ""}, {formatEquationLHS[A2[[2]], "", remVars], b2[[2]], ""}}]];

  sol2x2 = substStepsOne2[A2, b2, remVars];
  If[sol2x2 === $Failed, Return[$Failed]];

  With[{c2 = sol2x2["Content"]},
    content = Join[content, TakeWhile[c2, Not @ MatchQ[#, Style[s_String, Bold, ___] /; StringContainsQ[s, "Skúška správnosti"]] &]];
  ];
  ...
  AppendTo[content, alignedEquations[{
    {elimVar, tf[substExpr], substNote[solMap, remVars, {0, 0, 0}, remVars]},
    {elimVar, tf[filledExpr], ""},
    {elimVar, tf[Together[filledExpr]], ""}
  }]];
  ...
  finalVal = Table[If[vars[[i]] === elimVar, valElim, solMap[vars[[i]]]], {i, 1, 3}];
  content = addCorrectnessCheck[content, A, b, vars, finalVal];

  <|"Content" -> content, "Solution" -> finalVal|>
];

substStepsSingular3[A_, b_, vars_, kind_String, data_ : <||>] := Module[
  ...
  sol2x2 = substStepsSingular2[A2, b2, remVars, kind, False];
  If[sol2x2 === $Failed, AppendTo[content, "Pri riešení sústavy vznikol spor alebo identita."], content = Join[content, sol2x2["Content"]]];
...
substStepsNone3[A_, b_, vars_, data_ : <||>] := substStepsSingular3[A, b, vars, "NONE", data];
substStepsInfinite3[A_, b_, vars_, data_ : <||>] := substStepsSingular3[A, b, vars, "INFINITE", data];

(* ====================================================================== *)
(* ========================= MAIN RUNNER & PUBLIC ======================== *)
(* ====================================================================== *)

(* Config maps now refer to the flattened/prefixed function names *)

buildEquationSpecElimination[] := <|
  ...
  "RequireUnitCoeff" -> False,
  "StepsDispatch" -> Function[{dim, st},
    Switch[{dim, st},
      {2, "ONE"}, elimStepsOne2,
      {2, "NONE"}, elimStepsNone2,
      {2, "INFINITE"}, elimStepsInfinite2,
      {3, "ONE"}, elimStepsOne3,
      {3, "NONE"}, elimStepsNone3,
      {3, "INFINITE"}, elimStepsInfinite3,
      _, $Failed
    ]
  ]
|>;

buildEquationSpecSubstitution[] := <|
  ...
  "RequireUnitCoeff" -> True,
  "StepsDispatch" -> Function[{dim, st},
    Switch[{dim, st},
      {2, "ONE"}, substStepsOne2,
      {2, "NONE"}, substStepsNone2,
      {2, "INFINITE"}, substStepsInfinite2,
      {3, "ONE"}, substStepsOne3,
      {3, "NONE"}, substStepsNone3,
      {3, "INFINITE"}, substStepsInfinite3,
      _, $Failed
    ]
  ]
|>;

runEquationGenerator[spec_Association, diff_String, mode_String, opts___?OptionQ] := Module[{entryFn, msgPrefix, dimKey, dim, vars, stRaw, st, visQ, data, A, b, stepsFunc, steps, sol},
  ...
  dim = Switch[diff, "EASY", 2, "MEDIUM" | "HARD", 3, _, 3];
  vars = Take[{x, y, z}, dim];

  data = WithRetries @ Function[Null, generateLinearSystem[dim, diff, st, RequireUnitCoeff -> spec["RequireUnitCoeff"]]];
  ...
  stepsCounter = 0;
  stepsFunc = spec["StepsDispatch"][dim, st];
  If[stepsFunc === $Failed, Message[MessageName[msgPrefix, "fail"]]; Return[$Failed]];

  steps = If[dim == 3, stepsFunc[A, b, vars, data], stepsFunc[A, b, vars]];
  If[steps === $Failed, Message[MessageName[msgPrefix, "fail"]]; Return[$Failed]];
  ...
  If[mode =!= "TASK",
    ...
    Switch[sol,
      ...
      "INFINITE", printInfiniteResult[A, b, vars],
      _,
      CellPrint @ Cell[
        BoxData @ ToBoxes[
          If[dim == 2,
    ...
    If[visQ, If[dim == 2, visualize2[A, b, vars, sol], visualize3[A, b, vars, sol]]];
  ];

  sol
];

GenElimination[diff_String, mode_String, opts : OptionsPattern[]] := Module[{spec = buildEquationSpecElimination[]}, runEquationGenerator[spec, diff, mode, opts]];
GenSubstitution[diff_String, mode_String, opts : OptionsPattern[]] := Module[{spec = buildEquationSpecSubstitution[]}, runEquationGenerator[spec, diff, mode, opts]];
